...
func toAdaptWebExchange(echoc echo.Context) flux.WebExchange {
	...
	pkg.Assert(ok, fmt.Sprintf("invalid <request-resolver> in echo.context, was: %+v", echoc.Get(ContextKeyWebResolver)))
	server, ok := echoc.Get(ContextKeyWebBindServer).(flux.WebListener)
	pkg.Assert(ok, fmt.Sprintf("invalid <listen-server> in echo.context, was: %+v", echoc.Get(ContextKeyWebBindServer)))
	// 从Header中读取RequestId
	id := echoc.Request().Header.Get(echo.HeaderXRequestID)
	pkg.Assert("" != id, "invalid <request-id> in echo.context.header")
