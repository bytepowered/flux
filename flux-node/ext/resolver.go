...
var (
	mediaTypeValueResolvers = make(map[string]flux2.MTValueResolver, 16)
)

// RegisterMTValueResolver 添加实际值类型解析函数
func RegisterMTValueResolver(typeName string, resolver flux2.MTValueResolver) {
...
func MTValueResolverByType(typeName string) flux2.MTValueResolver {
	typeName = fluxpkg.MustNotEmpty(typeName, "typeName is empty")
	typeName = strings.ToLower(typeName)
